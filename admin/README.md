...
2. **본인을 관리자로 등록** — 같은 SQL editor에서 이메일만 본인 것으로 바꿔 실행하세요.
   ...
   **첫 관리자만 이렇게 넣으면 됩니다.** 두 번째부터는 관리자 화면의 **회원 관리**에서
   버튼으로 지정하고 뺄 수 있습니다(아래 "관리자를 더 넣기" 참고).

   SQL로 다뤄야 할 때를 위해 남겨둡니다. 뺀 사람은 그 즉시 막힙니다.
...
**회원 관리** 화면 맨 오른쪽에 `지정` / `해제` 버튼이 있습니다. 앱에 한 번이라도 로그인한
계정이면 여기서 바로 관리자로 만들 수 있고, 뺄 수도 있습니다. SQL editor를 열 필요가 없습니다.

지정하면 그 사람은 통계뿐 아니라 **회원 이메일과 가족 구성까지 모두** 보게 되고,
또 다른 사람을 관리자로 지정할 수도 있게 됩니다. 눌렀을 때 확인 창에 이 내용이 그대로 뜹니다.

막아둔 것이 둘 있습니다. 둘 다 화면이 아니라 데이터베이스 함수(`admin_set_admin`) 안에서
판단합니다. 화면에서 검사하면 두 사람이 동시에 눌렀을 때 둘 다 통과해버릴 수 있기 때문입니다.

- **자기 자신은 뺄 수 없습니다.** 실수로 스스로를 빼면 화면으로 다시 들어올 길이 없습니다.
- **마지막 한 명은 뺄 수 없습니다.** 명단이 비면 아무도 관리자 화면에 못 들어옵니다.

둘 중 하나에 걸리면 이유가 그대로 화면에 뜹니다. 정말 비워야 한다면 SQL editor에서
위의 `delete` 문을 쓰면 됩니다.

이 기능을 쓰려면 `supabase/admin-stats.sql`을 **다시 실행**하고
(`admin_set_admin` 함수가 새로 생깁니다) `admin-stats` 함수를 **다시 배포**해야 합니다.
...
- 누군가를 막아야 하면 `admin_users`에서 지우면 끝입니다. 재배포도, 다른 관리자의 재설정도
  필요 없습니다.
- **전체 데이터 초기화 기능은 없어졌습니다.** 테스트 기간에만 두었던 것이라 출시를
  앞두고 걷어냈습니다(`supabase/functions/reset-all-data`). 지울 일이 생기면 Supabase
  대시보드에서 직접 합니다.
