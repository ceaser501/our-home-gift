...
#
# 아이폰 판은 찍어온 폰의 크기 그대로다(828×1792, 아이폰 XR·11). 늘리지 않는 이유는
# 늘리면 흐려지기 때문이다 — 스토어 사진은 크게 보는 것이라 티가 난다.
#
# App Store Connect가 이 크기를 안 받으면 그때는 더 큰 아이폰에서 다시 찍어 이 값만
# 바꾼다. 받는 크기는 콘솔이 업로드 칸에 적어준다.
SETS = [
    ("갤럭시", RAW, OUT, (1080, 2160)),
    ("아이폰", os.path.join(RAW, "ios"), os.path.join(OUT, "ios"), (828, 1792)),
]
