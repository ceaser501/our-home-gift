...
// 카드의 기한 줄에 무엇이 서는지.
//
// 카드의 기한은 연도를 두 자리로 적는다(formatDateShortYear) — 자리가 빠듯하고
// 앞 두 자리는 어차피 20이라 아무것도 알려주지 않는다.
//
...
describe('못 쓰는 카드에는 D-day가 없다', () => {
  it('기한이 지난 카드 — 날짜만 남는다', () => {
    ...
    expect(screen.queryByText('20.01.01까지')).toBeTruthy();
  });
  ...
  it('사용완료 카드 — 기한이 남아 있어도 D-day를 안 적는다', () => {
    ...
    expect(screen.queryByText(/D-/)).toBeNull();
    expect(screen.queryByText('99.12.31까지')).toBeTruthy();
  });
