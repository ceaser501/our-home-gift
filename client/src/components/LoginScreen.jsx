...
// 지난번에 쓴 수단.
//
// 계정을 가르는 것은 로그인 수단이 아니라 이메일이다. 카카오로 들어오든 네이버로
// 들어오든 이메일이 같으면 같은 계정이고(네이버는 supabase/functions/naver-auth가 그
// 이메일로 매직링크를 발급하고, 구글·카카오는 Supabase가 검증된 같은 이메일에 붙인다),
// 다르면 아예 다른 계정이 된다 — 같은 사람이 가족 구성원 두 명으로 늘어난다.
//
// 한때 이 자리에 "다른 방법으로 로그인하면 새 계정이 만들어져요"라고 적었는데 그건
// 반만 맞는 말이었다. 카카오와 네이버에 같은 이메일을 쓰는 사람에게는 아무 일도 안
// 일어나는데 겁을 줬고, 정작 이메일이 다른 사람에게는 무엇이 기준인지 안 알려줬다.
//
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="flex items-start gap-2 px-0.5">
        ...
        {/* 겁만 주지 않는다. '같으면 같은 계정'까지 적어야 기준이 이메일이라는 것이
            드러나고, 카카오·네이버에 같은 이메일을 쓰는 사람은 안심하고 누를 수 있다. */}
        <p className="m-0 flex-1 text-[13px] leading-relaxed break-keep text-muted-foreground">
          이메일이 같으면 같은 계정, 다르면 새 계정이에요
        </p>
      </div>
    </div>
  );
