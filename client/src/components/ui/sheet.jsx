...
function SheetDescription({ className, ...props }) {
  ...
  // 회색이다. 색을 덮어쓰지 않는다.
  //
  // 검정은 값에, 회색은 값을 둘러싼 말에 쓴다. 그래서 여기에 검정으로 세우고 싶은
  // 것이 있으면 색을 바꾸는 것이 아니라 자리를 바꾼다 — 본문 첫 줄로 내린다.
  // 기한 연장 2/2 의 상품명이 그 예다. 저장을 누르기 직전에 확인하는 값이라
  // 본문에서 14 준굵게 검정으로 선다.
  //
  // 그 반대도 마찬가지다. 여기 앉은 것은 회색이 어울리는 말이어야 한다 — 무엇에
  // 대한 창인지(상품명·가족 이름) 또는 제목을 한 마디 보태는 설명이다.
  //
  // 자리에 규칙이 붙는 것이지 안에 든 것에 붙는 것이 아니다. 같은 상품명이라도
  // 부제에 있으면 회색이고 본문에 있으면 검정이다 — 자리가 무게를 정한다.
  return (
