...
// 리듬이 깨지고, 셋인지 다섯인지가 한눈에 안 잡힌다.
//
// 재는 자는 100%가 아니라 115%다. 폰 설정에서 글자를 키워둔 사람을 그만큼 따라가기
// 때문이다(MainActivity의 applySystemFontScale). 360px 폰에서 이 줄에 쓸 수 있는 폭이
// 268px이므로, 100% 기준으로는 233px 안에 들어와야 키워도 세 줄로 남는다.
// 여기 글자를 늘릴 때는 그 자로 재야 한다 — 아버지 세대는 대개 키워놓고 쓴다.
// '등록한'은 뺐다. 이 앱에 있는 건 다 등록한 것이라 하는 일이 없는 말이고, 그 세 글자를
// 빼야 '구성원'이 들어간다. 구성원 쪽은 값이 있다 — 앱이 쓰는 말이 '가족 구성원'이라
// 여기만 '가족에게'라고 하면 다른 것을 가리키는 것처럼 읽힌다.
const NOTICES = [
  '기프티콘은 가족 구성원에게 보여요',
  // '이미지가'를 뺐다. 앞의 '사진을'과 같은 것을 두 번 말하는 자리다.
  '사진을 읽을 때 국외로 전송돼요',
  '만 14세 미만은 이용할 수 없어요',
];
