...
// 찾은 것은 같은 자리에서 다시 찾지 않는다. 자리로만 판단하고 시간은 안 본다.
//
// '없더라'는 안 적는다. 그 답은 경계(500m)에 붙어 있어서 위치가 몇 미터만 달라져도
// 뒤집히는데, 적어두면 100m를 걷기 전에는 못 고친다. 아래 run() 안의 주석 참고.
//
...
export default function NearbyBanner({ gifticons, onPick }) {
  ...
  function search(knownAt = null) {
    ...
    async function run() {
      ...
      // 찾은 것만 적어둔다. '없더라'는 안 적는다.
      //
      // 둘의 수명이 다르다. 매장을 찾았다는 답은 오래 간다 — 매장은 움직이지 않으니
      // 같은 자리면 몇 시간 뒤에도 거기 있다. 그런데 '없다'는 답은 경계에 붙어 있다.
      // 500m가 기준인데 489m짜리 매장이 있으면 다음 위치가 11m만 달라져도 뒤집힌다.
      // 그 뒤집히는 답을 적어두면 100m를 걷기 전에는 못 고친다.
      //
      // 실제로 그랬다. 띠에는 '없어요'가 떠 있는데 카드의 '매장'을 누르면 489m·463m가
      // 나왔다. 띠는 아까 자리에서 적어둔 '없더라'를 그대로 쓰고 있었고, 매장 쪽은
      // 그 자리에서 새로 물어본 것이라 답이 갈렸다.
      //
      // 안 적으면 앱을 열 때마다 다시 물어본다. 그게 원래 정한 규칙이기도 하다 —
      // 앱을 열 때와 브랜드가 바뀔 때 찾는다. 검색은 사람당 하루 2,000회이고 한 번에
