...
//   우상단 — 그 모서리의 색 비율. 앱 화면은 거기가 닫기(X)나 검색 자리라 흰(또는 검은)
//            바탕에 무채색 획뿐이고, 발행 카드는 모서리까지 브랜드 색이 찬다.
//
//            태수님이 "우측 최상단에 X가 있으면 스크린샷"이라고 짚어준 자리다. 다만 X를
//            찾지는 않는다 — 획 자체는 잉크 0.037밖에 안 되고, 아이콘 모양은 제조사·
//            테마·언어마다 달라진다. 갈리는 것은 그 자리에 색이 있느냐다.
//
// ── 실측 (2026-08-20, 태수님이 올린 실물) ──
//                                 색      잉크   우상단    합
//   스벅 원본 800x1670           0.436   0.587   1.000   2.023
//   같은 기프티콘 선물하기 캡처   0.160   0.344   0.000   0.504   ← 갈라야 하는 짝
//   교촌 원본                    0.288   0.390   0.733   1.411
//   라떼 원본                    0.436   0.560   1.000   1.996
//   투썸 원본                    0.283   0.435   0.733   1.451
//   카드1618 원본                0.515   0.576   1.000   2.091
//   giftishow 450 원본           0.253   0.472   0.310   1.035
//   카톡이 줄여 보낸 배스킨       0.417   0.490   0.939   1.846
//   앱 화면 캡처(모아콘)         0.079     —     0.000
//   바코드만 크게 띄운 캡처       0.03    0.031   0.00    0.06
//   같은 것, 다크 모드            0.03    0.136   0.00    0.17
//
// 잉크만으로는 아슬아슬했다 — 원본 최저가 0.350인데 갈라야 할 캡처가 0.344다. 태수님이
// 올린 스벅은 0.587이라 갈렸지만 교촌이었으면 뒤집혔다. 색을 더하고 우상단까지 보면
// 원본 최저 1.035, 캡처 0.504로 두 배 넘게 벌어진다.
//
...
// 실측에서 갈라야 했던 두 장은 0.531 벌어져 있었다(원본 최저 1.035 ↔ 캡처 0.504).
const RICHNESS_TIE = 0.10;

// 우상단 모서리로 볼 자리. 닫기(X)나 검색이 놓이는 만큼만 잡는다.
const CORNER_X = 0.85;
const CORNER_Y = 0.05;
...
function infoRichness(image) {
  ...
  const cornerFrom = Math.round(canvasWidth * CORNER_X);
  const cornerRows = Math.max(1, Math.round(height * CORNER_Y));
  let colored = 0;
  let cornerColored = 0;
  let cornerTotal = 0;
  for (let i = 0; i < data.length; i += 4) {
    ...
    const hasColor = Math.max(r, g, b) - Math.min(r, g, b) > COLOR_EDGE;
    if (hasColor) colored += 1;

    const pixel = i / 4;
    if (pixel < cornerRows * canvasWidth && pixel % canvasWidth >= cornerFrom) {
      cornerTotal += 1;
      if (hasColor) cornerColored += 1;
    }
  }
  let peak = 0;
  ...
  const corner = cornerTotal > 0 ? cornerColored / cornerTotal : 0;
  return ink + colored / total + corner;
}
