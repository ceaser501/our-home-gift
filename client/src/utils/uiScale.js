...
// 폰마다 기준이 다르다. 갤럭시는 4% 내리고, 아이폰은 7% 올린다.
//
// 같은 CSS px가 두 폰에서 물리적으로 다른 크기로 그려진다. 갤럭시 S25는 기본 설정에서
// 1인치에 138px이 들어가고 아이폰 15/16은 154px이 들어간다 — 같은 15.5px 글자가
// 갤럭시에서 11% 크게 보인다는 뜻이다. 그래서 갤럭시에서는 폰에 깔린 다른 앱들보다
// 모아콘이 커 보였다. 카톡·토스와 나란히 놓고 대보고 안 것이다.
//
...
//
// ── 이 두 숫자를 계산으로 정하려다 두 번 실패했다 ──────────────────────────
//
// 갤럭시는 0.93에서 시작했다. 11% 차이를 그대로 메우는 값이라 계산상 맞았는데 폰에
// 깔고 보니 작았다. 0.96 → 0.98로 올렸다가, 다시 0.96으로 내려왔다.
//
// 아이폰은 반대로 1.05에서 시작해 1.10까지 올렸다. 두 폰을 물리적으로 같게 만드는
// 자리가 0.98 × 154/138 = 1.09라서 그 값으로 간 것인데, 이번엔 컸다. 1.05로 돌아왔다가
// 폰에서 한 번 더 보고 1.07로 올렸다.
//
// 두 번 다 계산이 가리킨 곳을 지나쳐 갔다가 눈이 있는 자리로 돌아왔다. 물리적으로 같은
// 크기가 좋은 크기는 아니었던 셈이다 — 폰을 쥐는 거리도, 화면에 담기는 카드 수도 그
// 계산에는 안 들어간다. 다음에 이 숫자를 만질 일이 있으면 계산은 방향만 보고, 값은
// 폰에 깔아서 정한다.
const PLATFORM_BASE = { android: 0.96, ios: 1.07 };
