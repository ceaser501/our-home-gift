...
//
// 아이폰은 반대로 10% 올린다. 같은 이유의 뒷면이다 — 1인치에 154px이 들어가서
// 갤럭시보다 작게 그려진다. 두 폰이 물리적으로 같은 크기가 되는 자리를 계산하면
// 154/138 = 1.116이고, 갤럭시 기준 0.98에 곱하면 1.09다.
//
// 처음엔 1.05에서 끊었다. 계산값의 절반쯤에서 눈으로 보자는 것이었는데, 폰에 깔고 보니
// 바뀐 줄을 모를 만큼 티가 안 났다. 5%는 16px 글자가 16.8px 되는 정도다 — 나란히 놓고
// 대야 보이는 차이라, 한 대만 보는 사람에게는 안 한 것과 같다.
//
// 갤럭시를 2%씩 더듬어 내리던 것과 다르다. 그때는 목표가 "다른 앱들과 비슷하게"라서
// 눈이 유일한 자였다. 여기는 갤럭시라는 맞출 대상이 이미 있고 그 거리가 숫자로 나온다.
const PLATFORM_BASE = { android: 0.98, ios: 1.1 };
