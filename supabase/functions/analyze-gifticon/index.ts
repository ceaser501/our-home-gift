...
const PROMPT_VERSION = '2026-08-17-목록화면';
...
- 유효기간은 **그 날짜가 기한이라고 적혀 있을 때만** 쓴다. "유효기간", "사용기한",
  ...
  - **여러 기프티콘이 나열된 화면에서는 기한도 금액도 읽지 않는다.** 선물함 목록, 보관함
    목록, 카드가 위아래로 여러 장 늘어선 화면이 그렇다. 거기 적힌 "○○까지"는 옆 칸
    기프티콘 것일 수 있고, 어느 것이 이 상품 것인지 그 화면만 봐서는 알 수 없다.
    실제로 목록 화면 캡처에서 다른 기프티콘의 날짜가 들어와 등록되자마자 D-Day가 됐다.
    상품 하나만 크게 보여주는 화면(상세·쿠폰 그림)에서 읽는다.
