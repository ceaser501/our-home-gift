...
-- 누르면 그것으로 끝이었다. 되돌릴 수 없는 결정에 되돌리는 문이 없으면 승인 자체가
-- 무서운 일이 된다.
--
-- 대표는 "가장 먼저 들어온 사람"이다. 화면도 같은 순서로 첫 줄에 '대표'를 붙인다 —
-- 둘이 다른 사람을 대표로 알면 버튼은 보이는데 눌러도 안 되는 일이 생긴다.
--
-- 나가기(leave_family)와 같은 자리는 같게 둔다. 다만 메모·알림 기록·사용 기록은 여기서
-- 더 지운다. 스스로 나간 사람과 내보내진 사람은 사정이 다르다 — 후자는 애초에 여기
-- 있으면 안 되던 사람이라, 이름이 남아 있는 것 자체가 문제다.
-- 자세한 사연은 supabase/kick-member.sql에 적어뒀다.
...
-- 없다. 앱은 파이어베이스(FCM) 토큰으로 받는다. (따로 돌리는 판: native-push.sql)
...
-- 초대 코드로 가족 이름만 미리 물어본다.
--
-- 링크를 눌러 들어온 사람에게 「'우리집' 가족에 초대받았어요」라고 말하려면, 신청을
-- 넣기 전에 이름을 알아야 한다. 이름을 링크에 실어 보낼 수도 있었지만 그 값은 보내는
-- 사람이 마음대로 적을 수 있어서, 화면이 거짓말을 하게 된다.
--
-- 알려주는 것은 이름 하나다. 코드가 틀리면 아무것도 안 준다. 코드를 계속 넣어보며 남의
-- 가족 이름을 긁어모으는 것을 막아야 해서 신청과 같은 통으로 횟수를 센다.
-- 자세한 사연은 supabase/invite-peek.sql에 적어뒀다.
...
-- ── 쓴 사건을 한 줄씩 ───────────────────────────────────────────────────
--
-- 기프티콘 한 장이 곧 사용 내역 한 줄이던 때가 있었다. 5만원권을 엄마 3,000 · 딸 20,000 ·
-- 아들 1,500 · 마지막 25,500으로 나눠 써도 앱이 아는 것은 "쓴 금액 50,000" 하나뿐이라,
-- 마지막에 다 채운 사람이 5만원을 쓴 것으로 보였다. 앞의 세 번은 화면 어디에도 없었다.
--
-- 같은 원인으로 사용취소도 안 됐다. 마지막 한 번만 되돌리려 해도 그게 얼마였는지를 모른다.
--
-- 자세한 사연과 옮기는 SQL은 supabase/gifticon-uses.sql에 있다. 이 아래 세 덩어리
-- (표 · 트리거 · undo_last_use)는 그 파일에도 같은 내용이 있으니, 한쪽을 고치면
-- 다른 쪽도 같이 고친다.
create table if not exists public.gifticon_uses (
  id bigint generated by default as identity primary key,
  family_id uuid not null references public.families(id) on delete cascade,

  -- 기프티콘을 지우면 그 기록도 같이 지운다. 알림(activities)은 반대로 남겨두는데,
  -- 거기는 "무슨 일이 있었나"를 적는 자리라 지운 일도 일어난 일이다. 이 표는 "지금
  -- 우리 가족이 쓴 것들"을 세는 자리라, 목록에 없는 것이 결산에 남으면 숫자가 안 맞는다.
  gifticon_id bigint not null references public.gifticons(id) on delete cascade,

  -- 이름과 썸네일을 여기에도 적어둔다. 목록을 그릴 때마다 기프티콘 표를 붙일 일이 없고,
  -- 나중에 상품명을 고쳐도 그때 쓴 이름이 그대로 남는다.
  gifticon_name text not null,
  thumb_image_path text,

  -- 쓴 사람. 이름을 따로 적는 이유는 used_by_name과 같다.
...
-- 우리 가족 것만 본다. 쓰기 정책은 두지 않는다 — 트리거와 undo_last_use(security definer)만
...
begin
  ...
  -- 쓴 사건을 gifticon_uses에 한 줄. 알림과 달리 목데이터도 남긴다 — 목데이터로도
  -- 사용 내역 화면이 돌아야 확인할 수 있어서다.
...
-- 사용취소 — 마지막 한 번만 되돌린다.
--
-- 5만원권을 셋이 나눠 쓰고 내가 마지막 25,500원을 채웠다면 그 25,500원만 되돌린다.
-- 엄마 3,000 · 딸 20,000은 그대로 남는다. 전부 되돌리면 딸이 진짜로 쓴 2만원까지
-- 없던 일이 되는데, 사용취소를 누르는 상황은 대개 "방금 잘못 눌렀다"라서 남의 기록을
-- 지울 이유가 없다. 보통 기프티콘에는 이 구분이 없다 — 마지막 한 번이 곧 전부다.
--
-- 화면이 아니라 여기서 하는 이유는 셋이 한꺼번에 일어나야 해서다 — 줄 지우기,
-- 쓴 금액 되돌리기, 상태 바꾸기. 나눠 보내면 중간에 끊겼을 때 어긋난 채로 남는다.
