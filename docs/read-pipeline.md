...
## 목록 썸네일 — 모델이 짚어준 자리를 그대로 믿지 않는다

카톡 선물함 캡처를 68px로 줄이면 상품 사진·상품명·버튼이 한꺼번에 뭉개져 무엇인지
알아볼 수 없다. 그래서 상품 사진 부분만 잘라 따로 저장한다(`thumb_image_path`).

자리는 모델이 짚어준다(`analyze-gifticon`의 `thumbnail`, 백분율). **그런데 좌표는 글자보다
훨씬 자주 틀린다.** 실제로 목록에 이런 것들이 남았다.

| 카드 | 무엇이 썸네일이 됐나 |
|---|---|
| 배스킨라빈스 파인트 | 「유효기간 연장 및 환불이 불가합니다」가 적힌 **검은 약관 상자** |
| BBQ 황올반 | **바코드 블록** |
| 스타벅스 e-Gift | **안내 배너 전체** |

공통점이 있다 — 셋 다 **상품 사진이 작거나 없고, 옆에 큼직한 네모가 따로 있는** 캡처다.
「상품 사진이 있는 네모 영역」이라고만 물으면 모델은 화면에서 제일 큰 네모를 골라온다.

### 네 겹으로 거른다

**1. 무엇이 아닌지를 적어준다** (`analyze-gifticon/index.ts`)
바코드·QR 자리, 약관·안내 글자 상자, 로고 띠, 화면 전체는 상품 사진이 아니라고 명시하고,
그런 것밖에 없으면 `image`를 0으로 내라고 한다. **엉뚱한 데를 자르느니 안 자르는 게 낫다** —
안 자르면 목록은 캡처 전체를 보여주는데, 그건 적어도 무슨 카드인지는 알아볼 수 있다.

**2. 모델이 준 좌표를 검사한다** (`parseThumbnail`)
15% 미만, 양쪽 다 90% 초과, 가로세로 비 2.5배 초과, 화면 밖이면 버린다.

**3. 막대 자리와 겹치면 버린다** (`imageAnalyze.js`의 `boxHolds`) ← **가장 단단한 겹**
막대가 어디 있는지는 zxing이 이미 읽어뒀다(`prepareImages`의 `barcodeBox`). 짐작이 아니라
**잰 값**이라 프롬프트보다 믿을 만하다. 막대의 가운데가 썸네일 네모 안에 들면 같은 자리를
가리킨 것으로 본다 — 1D 막대는 납작하고 QR은 네모라 모양이 제각각인데, 가운데 한 점이면
둘 다 똑같이 잴 수 있다.

**4. 잘라낸 자리가 글자판이면 버린다** (`looksLikeTextPanel`)
글자판은 두 가지가 함께 나타난다 — 바탕색 하나가 밝기의 한 칸에 몰려 있고(65% 이상),
색이랄 게 없다(채도 있는 점이 12% 미만). **둘 다 걸릴 때만** 버린다. 하나만 보면 흰 접시에
담긴 검은 커피 같은 것이 억울하게 걸린다. 한 변에 40칸씩 1,600점만 훑어서 수십 ms로 끝난다.

### 정사각형으로 맞춘다

목록 칸은 정사각형이다(`GifticonCard`의 62×62). 잘라낸 그림이 가로로 길면 그 칸이 양옆을
**다시** 잘라내서, 상품이 한쪽에 치우쳐 있으면 화면에서 사라진다. 그래서 자를 때 짧은 쪽을
가운데 기준으로 늘려 정사각형으로 맞추고, 사진 가장자리에 닿으면 그만큼 안쪽으로 민다
(`toSquare`).

### 값은 그대로다

넷 다 **돈이 안 든다.** 프롬프트에 문장이 늘어도 요금은 사진 픽셀 수가 정하고, 3·4번은
브라우저에서 도는 계산이다. 늘어나는 시간은 색을 재는 수십 ms뿐 — 한 건 읽는 데 4초쯤
걸리니 티가 안 난다.

### 다시 자를 길은 아직 없다

한 번 잘못 잘리면 되돌릴 방법이 없다. 잘라둔 그림만 남고 **원래 네모 좌표는 저장하지 않기**
때문에, 다시 자르려면 원본을 모델에게 한 번 더 보내야 한다(건당 6원). 카드 메뉴에
「썸네일 다시 잡기」를 두는 안이 있고, 위 넷이 잘 들으면 필요가 줄어든다.
