...
| <img src="images/gift-screenshot.png" width="150"><br>**10. 선물하기 캡처**<br>읽힘. 대표는 원본에 양보 | <img src="images/giftishow-biz.png" width="150"><br>**기프티쇼 biz**<br>3번과 같은 갈래 | <img src="images/text-only-coupon.png" width="150"><br>**8. 글자만 있는 쿠폰**<br>막대가 없다. 서버가 쿠폰번호를 | <img src="images/app-list.png" width="150"><br>**12. 목록 화면 캡처**<br>여러 건이 한 화면. **뺀다** |

**8번이 이번 작업(2-5)의 이유가 된 유형이다.** 막대가 아예 없어서 브라우저는 아무것도
못 보고, 그림만 봐서는 곁가지 캡처와 구별되지 않는다.

**12번**의 썸네일 속 막대는 40픽셀이라 찍힐 수가 없어 가리지 않았다. 이 화면을 통째로
찍으면 zxing이 그 썸네일에서 번호를 읽어내는데, 옆칸 기프티콘의 금액과 기한이 같이
들어온다 — 그래서 뺀다(2-2).
...
- [x] 3·4번(원본 + 정보 캡처) 등록 폼, 1건, **금액 채워짐** — 두 번 깨졌던 자리다
- [x] 6번(원본 + 파인트) **2건으로 나뉨**
- [x] 9번 AddCON 상품명 (프롬프트 손보고 재배포한 뒤)
- [x] 7번 QR — 저장·재생성, 다건에 섞어도 됨
- [x] 5번 스타벅스 e-Gift — 기한은 빈칸, 얼럿으로 채움
- [x] 8번 배스킨 — 다건에서도 잡힘
- [x] 자동스캔 그대로 돔

### 남은 하나 ★

- [x] **매장에서 실제로 결제됐다** (2026-09-14). 우리가 다시 그린 막대를 리더기가 읽었다

우리가 다시 그린 그림을 매장 리더기가 읽는지는 **매장에서만 알 수 있다.** 자동 시험도
실물 사진도 여기까지는 못 간다. 두 번 중 하나라도 안 되면 출시를 미뤄야 하는 항목이다.

| 무엇으로 | 어디서 | 무엇을 보나 |
| --- | --- | --- |
| 바코드 기프티콘 하나 | 아무 매장 | 앱 화면을 리더기에 대고 찍히는지 |
| QR 기프티콘 하나 | 편의점 | 같은 방식. `IX;1;…;;` 껍데기가 그대로 전달되는지 |

안 읽히면 번호를 직접 불러 결제해보고, **번호로는 되는지**까지 확인해주세요.
그 답에 따라 고칠 자리가 갈립니다 — 그림을 못 그리는 것인지, 형식이 다른 것인지.
