# 스토어 등록정보에 넣을 것

Play Console의 「앱 설정 완료」와 「스토어 등록정보」에 붙여넣을 문구와 답변을 모아둔다.
App Store에도 거의 그대로 쓴다.

- 정리 시점: 2026-09-02
- 앱 이름 **모아콘** · 패키지 `io.github.ceaser501.ourhomegift`

## 앱 이름

```
모아콘
```

## 짧은 설명 (80자)

```
가족이 함께 쓰는 기프티콘 서랍. 사진만 올리면 알아서 정리되고, 기한 전에 알려드려요.
```

## 자세한 설명 (4000자)

```
받아놓고 잊어버린 기프티콘, 몇 장이나 되시나요.

모아콘은 가족이 기프티콘을 한곳에 모아두고 함께 쓰는 앱입니다.
카카오톡으로 받은 사진을 올리기만 하면 나머지는 알아서 채워집니다.


■ 사진만 올리면 끝

바코드와 QR을 읽고, 상품명·상호·금액·유효기한까지 자동으로 채웁니다.
직접 입력할 것이 거의 없습니다. 틀린 곳만 고쳐주세요.


■ 사진첩에서 알아서 찾아줍니다

기프티콘이 사진첩 어딘가에 묻혀 있어도 괜찮습니다.
다운로드·카카오톡·스크린샷 폴더를 훑어 기프티콘으로 보이는 것만 골라 보여드립니다.
한 번 아니라고 하신 사진은 다시 묻지 않습니다.


■ 기한을 놓치지 않게

기한이 가까운 순서로 목록을 보여드리고, 만료 전에 알림을 보내드립니다.
안 쓰고 버리는 일이 줄어듭니다.


■ 가족이 함께

초대 코드나 카카오톡 링크로 가족을 부르면 같은 목록을 함께 봅니다.
누가 받은 것인지, 누가 쓸 것인지 표시해두고, 쓴 내역도 남습니다.
같은 기프티콘을 둘이 쓰러 가는 일이 없어집니다.


■ 매장에서 바로

바코드를 크게 띄워 리더기에 대면 됩니다.
금액권은 쓴 만큼 적어두면 잔액이 계산됩니다.
가까운 사용처를 지도에서 찾아볼 수도 있습니다.


■ 이런 분께

· 기프티콘을 자주 받는데 기한을 넘겨 버린 적이 있는 분
· 가족끼리 나눠 쓰고 싶은데 누가 뭘 갖고 있는지 모르는 분
· 사진첩에 기프티콘이 흩어져 있는 분


모아콘은 기프티콘을 사고팔지 않습니다. 이미 갖고 계신 것을 정리해드릴 뿐입니다.


─────────────────────────────
화면과 스크린샷에 보이는 상호는 이용자가 등록한 기프티콘의 정보를 그대로 보여준 것이며,
모아콘은 해당 브랜드와 제휴·후원 관계가 없습니다.
```

## 앱 액세스 권한 — ⚠️ 빠뜨리면 반려된다

이 앱은 로그인해야 화면이 보인다. **심사자가 들어올 수 있는 계정을 줘야 한다.**
안 주면 "앱을 볼 수 없다"로 반려된다.

문제가 하나 있다. 이메일 매직 링크는 심사자가 그 메일함을 열 수 없어서 쓸 수 없다.
**심사용 구글 계정을 하나 만들어 아이디와 비밀번호를 주는 편이 확실하다.**

- [x] 심사용 구글 계정 — `moacon.review@gmail.com` (2026-09-12)
- [x] 그 계정에 「테스트 가족」과 기프티콘 3개를 넣어뒀다
      (빈 화면만 보이면 기능을 확인할 수 없어 반려될 수 있다)

**비밀번호는 콘솔에 직접 친다.** 이 파일은 공개 저장소에 있다.

안내문에 적을 내용:

```
로그인이 필요한 앱입니다. 시작 화면에서 「구글로 로그인」을 눌러 들어와 주세요.

로그인하면 「테스트 가족」과 등록된 기프티콘 3개가 바로 보입니다.

사진첩을 훑어 기프티콘을 찾아주는 기능은 사진 권한을 허용해야 동작합니다.

위치 권한은 주변 매장을 찾는 그 순간에만 쓰고 저장하지 않습니다.
```

## 데이터 보안 (Data safety)

개인정보처리방침(`client/public/privacy.html`)과 **어긋나면 안 된다.** 거기 적힌 그대로 답한다.

| 항목 | 수집 | 공유 | 비고 |
|---|---|---|---|
| 개인 정보 → 이메일 주소 | 예 | 아니요 | 계정 관리·앱 기능 |
| 개인 정보 → 이름 | 예 | 아니요 | 가족 안에서 쓰는 표시 이름 |
| 사진 및 동영상 → 사진 | 예 | 예 | 앱 기능. 자동 인식 때 Anthropic으로 전송 |
| 위치 → 대략적/정확한 위치 | 예 | 예 | **「일시적으로만 처리하고 저장하지 않음」에 체크.** 매장 찾기에서 카카오·TMAP로 보낸다 |
| 앱 활동 → 기타 사용자 생성 콘텐츠 | 예 | 아니요 | 메모·표시 이름 |
| 기기 또는 기타 ID | 예 | 아니요 | 알림 보낼 때 쓰는 푸시 토큰 |

보안 관행

- **전송 중 데이터 암호화: 예** (전부 HTTPS)
- **데이터 삭제 요청 가능: 예** (앱 안에서 내 메뉴 → 계정 삭제)

## 콘텐츠 등급

설문에 정직하게 답하면 전체이용가가 나온다.

- 폭력·성적 내용·욕설·약물·도박: 전부 **없음**
- 사용자 간 콘텐츠 공유: **있음** — 같은 가족 안에서만 보인다. 공개 게시판이 아니다
- 개인정보 공유: 없음
- 위치 공유: 없음 (매장을 찾는 그 순간에만 쓰고 저장하지 않는다)

## 타겟층 및 콘텐츠

- **만 18세 이상만 고른다.** 13~15세나 16~17세를 같이 넣으면 미성년자 대상 앱이 되어
  Play의 가족 정책이 걸린다 — 앱 설계·광고·데이터 수집 기준이 달라지고, 위치 권한을
  쓰는 앱이라 심사가 훨씬 까다로워진다.
- 넓게 잡는다고 얻는 것이 없다. 이 칸은 **누구를 대상으로 만들었나**를 적는 곳이지,
  그 나이가 아니면 설치를 못 하게 막는 곳이 아니다.
- 개인정보처리방침은 만 14세 이상 이용 가능으로 적혀 있다(`client/public/privacy.html:107`).
  **이용 가능 연령과 대상 연령은 다른 값이라 어긋나는 것이 아니다.**
- 아동을 대상으로 하지 않음 · 아동의 관심을 끌 수 있는지: **아니요**
- 광고: **없음**

> 처음에 여기 「만 13세 이상」이라고 적어두었다가 콘솔에서 13~15세 한 구간만 골라
> 제출되는 일이 있었다. 2026-09-04에 고쳤다.

## 그 밖의 선언

- 정부 앱: 아니요
- 금융 기능: **아니요** — 기프티콘을 사고팔지 않는다. 이 점을 분명히 해두는 편이 안전하다
- 건강 앱: 아니요

## 개인정보처리방침 URL

```
https://ceaser501.github.io/our-home-gift/privacy.html
```

## 그래픽

저장소에 있다.

| 쓰임 | 파일 |
|---|---|
| 앱 아이콘 512 | `assets/app-icons/store/play-store-512.png` |
| 그래픽 이미지 1024×500 | `assets/marketing/play-feature-graphic-1024x500.png` |
| App Store 아이콘 1024 | `assets/app-icons/store/icon-1024.png` |

**스크린샷 8장은 `assets/marketing/screenshots/`에 있다.** 1080×2160 PNG, 그대로 올리면 된다.

## 스크린샷 8장

| # | 파일 | 화면 |
|---|---|---|
| 1 | `assets/marketing/screenshots/01-list.png` | 목록 |
| 2 | `assets/marketing/screenshots/02-scan.png` | 사진첩 훑는 중 |
| 3 | `assets/marketing/screenshots/03-form.png` | 자동으로 채워진 등록 화면 |
| 4 | `assets/marketing/screenshots/04-barcode.png` | 상세 · 바코드 |
| 5 | `assets/marketing/screenshots/05-push.png` | 알림 (푸시 두 종류) |
| 6 | `assets/marketing/screenshots/06-map.png` | 근처 매장 · 길찾기 |
| 7 | `assets/marketing/screenshots/07-invite.png` | 가족 초대 |
| 8 | `assets/marketing/screenshots/08-stats.png` | 사용 내역 |

**바코드 번호는 지웠다.** 원본에 살아 있는 BBQ 기프티콘 번호가 그대로 찍혀 있었다.
스토어 사진은 누구나 보고 확대할 수 있어서, 번호와 막대를 다른 값으로 바꿔 넣었다.
다음에 다시 찍을 때도 **이미 쓴 기프티콘으로 찍는다.**

아래는 그 8장에 얹은 문구다. Play는 스크린샷마다 설명을 적는 칸이 없어서 이미지에 넣었다.

### 얹은 문구

목록의 앞 두세 장이 스토어에서 접히지 않고 먼저 보인다. 순서를 바꾸지 않는다.

| # | 찍을 화면 | 큰 글씨 | 작은 글씨 |
|---|---|---|---|
| 1 | 기프티콘 목록 | 기한이 급한 것부터 위로 | 며칠 남았는지 한눈에 보여요 |
| 2 | 사진첩 훑는 중 (안드로이드) | 찾는 것부터 앱이 합니다 | 사진첩을 훑어 기프티콘만 골라내요 |
| 3 | 자동으로 채워진 등록 화면 | 사진만 올리면 끝 | 상품명 · 금액 · 유효기한이 저절로 |
| 4 | 상세 · 바코드 | 매장에선 대기만 하면 돼요 | 금액권은 쓴 만큼 잔액이 계산돼요 |
| 5 | 푸시가 뜬 잠금화면 | 중요한 건 푸시알림으로 알려드려요 | 기한 만료 · 가족 참여 신청 |
| 6 | 근처 매장 지도 · 길찾기 | 쓸 수 있는 곳을 지도에서 | 길찾기 · 전화까지 바로 |
| 7 | 가족 초대 | 가족이 같은 서랍을 봅니다 | 카카오톡 링크 하나로 초대 |
| 8 | 사용 내역 | 우리 집이 얼마나 아꼈나 | 누가 언제 썼는지 그대로 남아요 |

2번은 훑는 도중 화면이다. 결과 목록보다 이쪽이 낫다 — 스캔 그림과 진행 막대가
있어서 "앱이 알아서 한다"가 한눈에 보이고, 「다운로드 · 카카오톡 · 스크린샷, 이 세
곳만 봐요」 한 줄이 화면 안에 이미 들어 있다.

**2번은 안드로이드에만 있다.** 사진첩을 훑는 기능은 iOS에 없다. App Store는 스크린샷을
따로 올리므로, 거기서는 2번을 빼고 7장으로 낸다. Play는 8장 그대로.

5번은 **참여 신청 알림만 실물이다.** 기한 만료 알림은 그 순간을 붙잡아야 찍을 수 있어서
못 찍었고, 찍힌 카드를 알약 모양으로 오려 아래에 붙이고 글자만 다시 썼다. 아이콘·화살표·
반투명 정도가 저절로 맞아서 손으로 그린 티가 안 난다.

**글자는 지어낸 것이 아니다.** `supabase/functions/send-expiry-notifications/index.ts:153`이
실제로 보내는 형식 그대로다 — 가족 이름 · 제목, 상호 · 상품명, 기한과 남은 날. 그 문구를
고치면 이 그림도 같이 고쳐야 한다. 만드는 코드는 대화 밖 임시 폴더에 있으니, 다시 만들
일이 생기면 찍힌 카드 위치(`CARD`)와 내릴 거리(`DROP`)만 다시 재면 된다.

앱이 보내는 알림은 이 둘뿐이다(`send-expiry-notifications`, `notify-join-request`).

5번의 「알림」은 **푸시**다. 헤더의 종은 가족이 올리거나 쓴 소식과 공지가 쌓이는 자리라
기한과 관계가 없다(`client/src/components/NotificationBell.jsx:17`).

### 다시 찍는다면

- **이미 쓴 기프티콘으로 찍는다.** 살아 있는 번호는 스토어에 올리면 안 된다.
- **상호.** 남의 브랜드가 사진에 남는다. 자세한 설명 맨 아래 고지 한 줄이 그 때문이다.

---

# App Store 전용 (2026-09-12 정함)

Play와 대부분 같지만 **설명 한 군데가 다르다.** 「사진첩에서 알아서 찾아줍니다」는
안드로이드에만 있는 기능이라 뺀다 — 없는 기능을 적으면 심사자가 찾다가 못 찾는다.
그 자리에 iOS에서 실제로 되는 것(여러 장 한 번에 올리기, 같은 바코드 알려주기)을 적었다.

## 부제 (30자)

```
가족이 함께 쓰는 기프티콘 서랍
```

## 프로모션 텍스트 (170자)

```
받아놓고 잊어버린 기프티콘, 이제 한곳에 모아두세요. 사진만 올리면 상품명·금액·유효기한이 저절로 채워지고, 기한 전에 알려드립니다. 가족이 같은 목록을 함께 봅니다.
```

## 설명 (4000자)

```
받아놓고 잊어버린 기프티콘, 몇 장이나 되시나요.

모아콘은 가족이 기프티콘을 한곳에 모아두고 함께 쓰는 앱입니다.
카카오톡으로 받은 사진을 올리기만 하면 나머지는 알아서 채워집니다.


■ 사진만 올리면 끝

바코드와 QR을 읽고, 상품명·상호·금액·유효기한까지 자동으로 채웁니다.
직접 입력할 것이 거의 없습니다. 틀린 곳만 고쳐주세요.


■ 여러 장을 한 번에

사진첩에서 여러 장을 골라 한꺼번에 올릴 수 있습니다.
이미 등록한 것과 바코드가 같으면 알려드려서, 같은 것을 두 번 넣는 일이 없습니다.


■ 기한을 놓치지 않게

기한이 가까운 순서로 목록을 보여드리고, 만료 전에 알림을 보내드립니다.
안 쓰고 버리는 일이 줄어듭니다.


■ 가족이 함께

초대 코드나 카카오톡 링크로 가족을 부르면 같은 목록을 함께 봅니다.
누가 받은 것인지, 누가 쓸 것인지 표시해두고, 쓴 내역도 남습니다.
같은 기프티콘을 둘이 쓰러 가는 일이 없어집니다.


■ 매장에서 바로

바코드를 크게 띄워 리더기에 대면 됩니다.
금액권은 쓴 만큼 적어두면 잔액이 계산됩니다.
가까운 사용처를 지도에서 찾아 길찾기와 전화까지 바로 할 수 있습니다.


■ 이런 분께

· 기프티콘을 자주 받는데 기한을 넘겨 버린 적이 있는 분
· 가족끼리 나눠 쓰고 싶은데 누가 뭘 갖고 있는지 모르는 분
· 사진첩에 기프티콘이 흩어져 있는 분


모아콘은 기프티콘을 사고팔지 않습니다. 이미 갖고 계신 것을 정리해드릴 뿐입니다.


─────────────────────────────
화면과 스크린샷에 보이는 상호는 이용자가 등록한 기프티콘의 정보를 그대로 보여준 것이며,
모아콘은 해당 브랜드와 제휴·후원 관계가 없습니다.
```

## 키워드 (100자)

```
기프티콘,모바일상품권,상품권,쿠폰,유효기한,기한알림,가족,공유,바코드,선물
```

**「기프티콘」은 SK텔레콤의 등록상표다**(2026-09-13 확인, store-release.md 6장).
09류 「내려받기 가능한 컴퓨터프로그램」이 지정상품에 들어 있어서 앱과 같은 자리다.

**키워드에는 남긴다.** 이 말로 찾는 사람이 대부분이고, 검색어로 쓰는 것과 이름으로
거는 것은 다르다. 다음 업데이트에서 「모바일상품권」을 맨 앞으로 옮겨 무게만 바꾼다.

```
모바일상품권,기프티콘,상품권,쿠폰,유효기한,기한알림,가족,공유,바코드,선물
```

**앱 이름은 다음 업데이트에서 바꾼다** — `모아콘 - 가족 모바일상품권 서랍`.
지금 심사 중인 제출본에 옛 이름이 들어가 있어서 건드리지 않는다.

## URL과 저작권

| 칸 | 값 |
|---|---|
| 지원 URL | `https://ceaser501.github.io/our-home-gift/intro.html` |
| 마케팅 URL | `https://ceaser501.github.io/our-home-gift/intro.html` |
| 버전 | `1.0` |
| 저작권 | `2026 Taesu Kim` |

지원 URL로 쓰는 intro.html에 문의 이메일을 넣었다 — `client/public/intro.html:576`.
사용자가 도움을 받을 곳이 적혀 있어야 하는 칸이라 비워둘 수 없었다.

## 앱 심사 정보 — 메모

```
로그인이 필요한 앱입니다. 시작 화면에서 「구글로 로그인」 또는
「Apple로 로그인」으로 들어와 주세요. 아래 계정은 구글 계정입니다.

로그인하면 「테스트 가족」과 등록된 기프티콘 3개가 바로 보입니다.

사진첩을 훑어 기프티콘을 찾아주는 기능은 안드로이드 전용입니다.
iOS에서는 사진을 직접 골라 등록합니다.

위치 권한은 주변 매장을 찾는 그 순간에만 쓰고 저장하지 않습니다.
```

사용자 이름은 `moacon.review@gmail.com`, 암호는 콘솔에 직접 넣는다.
연락처는 태수님 본인, 이메일은 `moacon.support@gmail.com`.

## 앱 정보 (왼쪽 메뉴)

| 칸 | 값 |
|---|---|
| 부제 | `가족이 함께 쓰는 기프티콘 서랍` |
| 카테고리 (기본) | 유틸리티 |
| 카테고리 (보조) | 라이프스타일 |
| 개인정보처리방침 URL | `https://ceaser501.github.io/our-home-gift/privacy.html` |
| 라이선스 계약 | 애플 기본(EULA) 그대로 |

카테고리를 「쇼핑」으로 두지 않는다. 기프티콘을 사고팔지 않는 앱이라 그 자리에 두면
기대와 다르고, 금융·상품권 쪽 심사 기준에 가까워진다. 「유틸리티」가 안전하다.

## 가격 및 사용 가능 여부

- **무료.** 무료로 만들면 유료로 못 바꾼다(인앱 결제와 구독은 무료 앱에서도 된다)
- 국가는 **대한민국만** 고른다. 화면이 한국어뿐이고 매장 검색도 국내 지도라, 밖에
  풀어두면 쓸 수 없는 사람이 받아보고 별점을 남긴다. 나중에 넓히는 것은 언제든 된다

## 앱이 수집하는 개인정보 (개인정보 라벨)

`client/public/privacy.html`에 적힌 것과 어긋나면 안 된다. 거기 Anthropic·카카오·TMAP가
모두 적혀 있는 것을 확인하고 맞췄다(2026-09-12).

**추적(Tracking)은 전부 「아니요」다.** 광고가 없고 제3자 추적도 없다.

| 데이터 | 목적 | 신원에 연결 | 추적 |
|---|---|---|---|
| 연락처 정보 → 이메일 주소 | 앱 기능 | 예 | 아니요 |
| 연락처 정보 → 이름 | 앱 기능 | 예 | 아니요 |
| 위치 → 정확한 위치 | 앱 기능 | 아니요 | 아니요 |
| 사용자 콘텐츠 → 사진 또는 비디오 | 앱 기능 | 예 | 아니요 |
| 사용자 콘텐츠 → 기타 사용자 콘텐츠 | 앱 기능 | 예 | 아니요 |
| 식별자 → 사용자 ID | 앱 기능 | 예 | 아니요 |
| 식별자 → 기기 ID | 앱 기능 | 예 | 아니요 |

- **위치는 저장하지 않는다.** 매장을 찾는 그 순간 카카오·TMAP로 보내고 버린다. 그래서
  신원에 연결되지 않는다. 다만 기기 밖으로 나가는 것은 맞으니 **수집으로 신고한다** —
  적게 신고했다가 걸리는 쪽이 많이 신고해서 손해 보는 쪽보다 나쁘다
- **사진은 공유에 해당한다.** 자동 인식 때 Anthropic으로 보낸다
- 기기 ID는 알림 보낼 때 쓰는 푸시 토큰이다

## 연령 등급

설문에 정직하게 답하면 낮은 등급이 나온다. 이 앱의 사실은 이렇다:

- 폭력·성적 내용·욕설·약물·도박·현실 도박: **전부 없음**
- 무제한 웹 접근: **없음** (앱 안에서 바깥 웹을 자유롭게 열지 않는다)
- 사용자 제작 콘텐츠: **있다** — 다만 같은 가족 안에서만 보인다. 공개 게시판이 아니고,
  모르는 사람끼리 만나는 자리가 없다. 대표가 승인해야 가족에 들어온다
- 메시지·채팅: **없음**
- 광고: **없음**

Play에서는 같은 답으로 전체이용가가 나왔다.

---

# Guideline 2.1 회신 (2026-09-13)

첫 제출이 "정보가 더 필요하다"로 돌아왔을 때 그대로 붙여넣을 글이다. 영어로 쓴다 —
심사팀이 읽는 글이다. **App Store Connect의 「앱 심사에 회신」과 「앱 심사 정보」의 메모
칸, 두 곳에 같은 내용을 넣으라**고 애플이 지정했다.

메모 칸은 4,000자까지다. 아래 글은 3,635자라 그대로 들어간다. 처음 쓴 글은 4,125자였고
거절당했다 — 여섯 항목을 하나도 빼지 않고 문장만 줄여 맞췄다.

메모 칸에 있던 한국어 안내는 지우고 이 글로 바꾼다. 담고 있던 내용(데모 계정 로그인,
사진첩 훑기는 안드로이드 전용, 위치는 저장하지 않음)이 3번 항목에 다 들어 있어서 잃는
것이 없고, 둘을 합치면 3,942자라 여유가 58자밖에 안 남는다. 심사팀이 읽는 글이니
한 벌로 두는 편이 낫다.

번호는 1부터 시작한다. 애플이 여섯 가지를 번호로 물었고 1번이 화면 녹화다.
`(link)` 자리에 영상 링크를 넣는다 — **로그아웃한 브라우저에서 열리는지 확인하고**
넣는다. 심사자가 못 여는 링크면 그대로 다시 돌아온다.

```
Thank you for the review. Here is the information you requested.

1. SCREEN RECORDING

(link)

2. PURPOSE AND TARGET AUDIENCE

Moacon is a shared drawer for mobile gift vouchers ("gifticons"), which are
widely used in Korea and arrive as images through messaging apps.

People receive these vouchers, forget them, and let them expire. When a family
receives several, nobody knows who holds what, and two people can walk to the
same store to use the same voucher.

The user uploads the voucher image; the app reads the barcode and fills in the
product name, brand, price and expiry date automatically. The list is sorted by
how soon each one expires, and a notification arrives before one does. A family
shares one list, so everyone sees what is available and who used what.

Target audience: adults in Korea, especially families who receive gift vouchers
often. The app is Korean-only.

The app does not sell, buy or broker gift vouchers, and handles no payments. It
only organises vouchers the user already owns.

3. HOW TO ACCESS THE MAIN FEATURES

Sign in with the demo account in App Review Information, using the "구글로
로그인" (Google) button on the first screen. "Apple로 로그인" also works. The
demo account already has a family with three gifticons, so the list is visible
right after signing in.

- Add: tap "+", choose a voucher photo, fields fill in automatically, save.
- Barcode: tap a card in the list.
- Find a store: tap "매장" on a card (needs location permission).
- Family: tap the people icon in the header.
- Usage history: profile menu (top right), then "사용 내역".
- Delete account: profile menu, then "계정 삭제".

Two notes. Scanning the photo library to find vouchers automatically is an
Android-only feature; on iOS the user picks each photo. Location is used only at
the moment a nearby store is searched, and is never stored.

4. EXTERNAL SERVICES

- Supabase - authentication, database, storage, serverless functions
- Anthropic (Claude API) - reads the uploaded voucher image and extracts the
  product name, brand, price and expiry date. This is the AI service behind the
  automatic fill. Images are sent only when the user uploads one.
- Kakao - login, Kakao Map (store search and map), Kakao Mobility (routes),
  and Kakao share for invites
- TMAP (SK Open API) - alternative route lookup
- Naver - login and product price lookup
- Google Sign-In, Sign in with Apple - authentication
- Firebase Cloud Messaging - push notifications

5. REGIONAL DIFFERENCES

Available in Korea only, Korean-only, and identical for every user there. Store
search uses Korean map data and is not useful elsewhere.

6. REGULATED INDUSTRY AND THIRD-PARTY MATERIAL

Not a regulated industry. Third-party material is map data from Kakao Map and
route data from Kakao Mobility and TMAP, used under registered developer keys
issued to this app by each provider under their terms of service.

Brand names on screen come from voucher images the user uploads. The app is not
affiliated with or endorsed by those brands, as stated in the description.

USER-GENERATED CONTENT AND ITS CONTROLS

Content is shared only inside a closed family group. There is no public feed, no
discovery, no comments, no messaging, and no way to encounter a stranger's
content.

- Joining requires an invite code AND approval by the family owner.
- The owner can remove any member at any time.
- Any member can leave; the vouchers they added are then hidden from the rest.
- Members delete their own content, and deleting the account removes all of it.

We are glad to add an explicit reporting control if you would like one.
```
