...
「기프티콘」은 상표로 등록된 이력이 있는 단어다(6장). 키프리스 확인 결과에 따라
「모바일상품권」 쪽으로 무게를 옮길 수도 있다.
...
⚠️ **지원 URL로 쓰는 intro.html에 문의 이메일이 없다.** 사용자가 도움을 받을 곳이
적혀 있어야 하는 칸이라, `moacon.support@gmail.com` 한 줄을 그 페이지에 넣는 편이 좋다.
웹은 main에 밀 때만 나가므로 같이 챙긴다.
